...
def install_missing_tools(missing_tools: List[str]) -> bool:
    ...
    for tool in missing_tools:
        if tool in config.TOOL_INSTALL_COMMANDS:
            ...
            try:
                subprocess.run(
                    config.TOOL_INSTALL_COMMANDS[tool].split(),
                    check=True,
                    capture_output=True
                )
    ...
    try:
        ...
        if result.returncode != 0:
            if verbose:
                ...
                logger.warning("isort found import ordering issues. Run with --verbose for details.")
            return False
        else:
        ...
        if discord_bot_fixer_path.exists():
            ...
            if target_path.is_file():
                ...
                should_run_discord_bot_fixer = (
                    "discord_bot" in target or 
                    "wdbx_plugins" in target or
                    Path(target, "discord_bot.py").exists() or
                    Path(target, "wdbx_plugins").exists()
                )
...
def parse_arguments():
    ...
    parser.add_argument('--tools', '-l', default='all',
                        help='Comma-separated list of tools to run (default: all)')
    parser.add_argument('--report', '-r', action='store_true',
                        help='Generate HTML report for WDBX linters')
    parser.add_argument('--install-missing', '-i', action='store_true',
                        help='Try to install missing tools')
