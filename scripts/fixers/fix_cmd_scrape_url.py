...
def fix_cmd_scrape_url():
    ...
    for i in range(len(lines)):
        if "print(\"You can disable robots.txt checking with: scrape:config respect_robots_txt=false\")" in lines[i]:
            # Check next line for duplicate
            if i + 1 < len(lines) and "print(\"You can disable robots.txt checking with: scrape:config respect_robots_txt=false\")" in lines[i + 1]:
