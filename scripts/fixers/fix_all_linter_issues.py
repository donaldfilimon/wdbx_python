...
def main():
    ...
    if not integrated_linter_path.exists():
        ...
        if not run_linters_path.exists():
            logger.error("Could not find linting scripts. Make sure you're running this from the project root.")
            return 1
    ...
    response = input("Do you want to proceed with fixing linter issues? [y/N] ").lower()
    if response != 'y':
