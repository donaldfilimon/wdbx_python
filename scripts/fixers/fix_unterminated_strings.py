        ...
        if idx < len(lines):
            ...
            if "print(" in lines[idx] and "\n" in lines[idx]:
                ...
                if "requests_per_minute" in lines[idx]:
                    lines[idx] = 'print("You can decrease request frequency with: scrape:config requests_per_minute=<lower_value>")\n'
                
                # Case 2: Line 1531 - request frequency in cmd_scrape_site
                elif "requests_per_minute" in lines[idx]:
                    lines[idx] = 'print("You can decrease request frequency with: scrape:config set requests_per_minute <lower_value>")\n'
                
                # Case 3: Line 1559 - max_pages warning
                elif "max_pages" in lines[idx] and "must be positive" in lines[idx]:
                    lines[idx] = 'print(f"\\033[1;33mWarning: max_pages must be positive, using default ({max_pages})\\033[0m")\n'
                
                # Case 4: Line 1559 - max_pages error 
                elif "max_pages" in lines[idx] and "must be an integer" in lines[idx]:
                    lines[idx] = 'print(f"\\033[1;31mError: max_pages must be an integer, using default ({max_pages})\\033[0m")\n'
                
                # Case 5: Line 1559 - chunks
                elif "chunks" in lines[idx]:
                    lines[idx] = 'print(f"{i + 1}. {item.get(\'url\', \'N/A\')} ({len(item.get(\'chunks\', []))}) chunks")\n'
