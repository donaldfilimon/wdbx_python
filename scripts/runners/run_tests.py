        ...
        if os.path.isdir(dir_path):
            for file in os.listdir(dir_path):
                if (
                    ...
                    and pattern.lower() in file.lower()
                ):
...
def list_available_tests() -> None:
    ...
    for category, dir_path in test_dirs.items():
        if os.path.isdir(dir_path):
            test_files = [
                f for f in os.listdir(dir_path) if f.startswith("test_") and f.endswith(".py")
            ]
            if test_files:
                print(f"\n{category}:")
...
def main() -> int:
    ...
    test_group.add_argument(
        "-m", "--markers", help="Run tests with specific markers (comma-separated)"
    )
    test_group.add_argument(
        "-M", "--skip-markers", help="Skip tests with specific markers (comma-separated)"
    )
