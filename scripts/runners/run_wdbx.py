...
def get_platform_info() -> Dict[str, str]:
    ...
    try:
        ...
        return {"system": "Unknown", "python_version": ".".join(map(str, sys.version_info[:3]))}
...
def check_data_directory(wdbx_dir: Path) -> bool:
    ...
    data_dirs = [wdbx_dir.parent.parent / "wdbx_data", Path.home() / ".wdbx" / "data"]
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--interactive", "-i", action="store_true", help="Run WDBX in interactive mode"
    )
    ...
    parser.add_argument("--no-socket", action="store_true", help="Do not start the socket server")
    ...
    parser.add_argument(
        "--check", action="store_true", help="Check environment and exit without running WDBX"
    )
...
def main() -> int:
    ...
    logger.info(
        f"Running on {platform_info['system']} {platform_info.get('release', '')} with Python {platform_info['python_version']}"
    )
