...
def get_async_client(
    wdbx_instance: Optional[WDBXCore] = None,
    max_workers: int = 4
) -> AsyncWDBXClient:
    """
    ...
    if _ASYNC_CLIENT_INSTANCE is None:
        _ASYNC_CLIENT_INSTANCE = AsyncWDBXClient(
            wdbx_instance=wdbx_instance or get_wdbx_core(),
...
async def example_async_usage():
    ...
    with LogContext(component="AsyncAPI"):
        ...
        block = await client.create_block(
            data={"name": "Test Block", "tags": ["test"]},
            embeddings=[vector]
        )
