    ...
    def validate(self, value: Any) -> List[str]:
        ...
        if self.value_type is not None:
            if self.value_type == bool and isinstance(value, str):
                ...
                errors.append(
                    f"Config '{self.key}' should be of type {self.value_type.__name__} "
...
class ConfigManager:
    ...
    def __init__(
        ...
        self.config_dir = (
            Path(config_dir)
            if config_dir
            else Path(os.environ.get("WDBX_CONFIG_DIR", Path(__file__).parent / "configs"))
        )
...
DEFAULT_CONFIGS = [
    ...
    ConfigItem(
        key="debug_mode", default=False, value_type=bool, description="Whether to run in debug mode"
    ),
    ConfigItem(
        key="api_key",
        default=None,
