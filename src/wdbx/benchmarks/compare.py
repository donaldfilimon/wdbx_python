...
class DatabaseBenchmarker:
    ...
    async def insert_vectors(self, vectors: NDArray[np.float32], batch_size: int) -> BenchmarkResult:
        """
            ...
            for name, benchmarker in self.benchmarkers.items():
                ...
                for i in range(self.config.iterations):
                    try:
                        ...
                        # Run benchmark
                        logger.info(f"Running {name} insertion benchmark (iteration {i+1}/{self.config.iterations})")
        ...
        for db_name, benchmarker in self.benchmarkers.items():
            ...
            for batch_size in self.config.batch_sizes:
                for top_k in self.config.top_k_values:
                    logger.info(f"Running {db_name} query benchmark with batch size {batch_size}, top-k {top_k}")
                    
                    # Run multiple iterations
                    for i in range(self.config.iterations):
                        try:
                            logger.info(f"Running {db_name} query benchmark (iteration {i+1}/{self.config.iterations})")
                            result = await benchmarker.query_vectors(query_vectors, top_k, batch_size)
                            self.results.append(result)
                        except Exception as e:
...
async def main() -> None:
    ...
    parser.add_argument("--num-vectors", type=int, default=100000, help="Number of vectors to benchmark with")
    parser.add_argument("--query-count", type=int, default=100, help="Number of queries to run")
    parser.add_argument("--batch-sizes", type=str, default="1,10,100,1000", help="Comma-separated list of batch sizes")
    parser.add_argument("--top-k-values", type=str, default="1,10,50,100", help="Comma-separated list of top-k values")
    parser.add_argument("--databases", type=str, default="wdbx,faiss,qdrant,milvus", help="Comma-separated list of databases to benchmark")
