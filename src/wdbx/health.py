...
class HealthMonitor:
    ...
    def check_disk_space(self) -> HealthCheck:
        ...
        try:
            ...
            description = (
                f"Sufficient disk space available: {free_gb:.2f} GB free ({free_percent:.1f}%)"
            )
    ...
    def check_memory_usage(self) -> HealthCheck:
        ...
        try:
            ...
            status = HealthStatus.OK
            description = (
                f"Normal memory usage: {memory_usage_mb:.1f} MB ({process_percent:.1f}% of system)"
            )
...
def get_health_endpoint_handler(wdbx_instance):
    ...
    async def health_handler(request):
        ...
        try:
            ...
            return {"status": status_code, "body": response_data}
