...
from ..ml.backend import BackendType, MLBackend, get_ml_backend
...
class TestMLBackend(WDBXTestCase):
    ...
    def test_cosine_similarity(self):
        ...
        self.assertAlmostEqual(
            self.backend.cosine_similarity(self.vector1, self.vector2), 0.0, places=6
        )

        # Same vector should have similarity 1
        self.assertAlmostEqual(
            self.backend.cosine_similarity(self.vector1, self.vector1), 1.0, places=6
        )
    ...
    def test_batch_cosine_similarity(self):
        ...
        self.assertAlmostEqual(similarities[3], 0.7 / np.sqrt(0.98), places=4)  # partial similarity
    ...
    def test_search_vectors(self):
        ...
        similarities, indices = self.backend.search_vectors(self.vector1, self.batch, top_k=2)
        ...
        self.assertAlmostEqual(similarities[1], 0.7 / np.sqrt(0.98), places=4)
