    ...
    def commit_transaction(self, transaction_id: str) -> bool:
        ...
        for key in transaction.write_set:
            if key in self.committed_versions:
                ...
                conflicting_versions = [v for v in self.committed_versions[key].keys() 
                                       if v > transaction.version]
                if conflicting_versions:
                    logger.warning(f"Write-write conflict detected for key {key} in transaction {transaction_id}")
                    transaction.abort()
                    return False
