...
MEMORY_OPTIMIZATION_ENABLED = os.environ.get("WDBX_MEMORY_OPTIMIZATION_ENABLED", "true").lower() in ("true", "1", "yes", "y")
...
class MemoryMonitor:
    ...
    def __init__(
        ...
        logger.info(f"Initialized memory monitor with max_percent={self.max_memory_percent}, "
    ...
    def should_optimize(self) -> bool:
        ...
        if system_percent > self.max_memory_percent or process_percent > (self.max_memory_percent * 0.8):
            logger.warning(f"Memory threshold exceeded - System: {system_percent:.1f}%, "
    ...
    def optimize_memory(self) -> Dict[str, Any]:
        ...
        logger.info(f"Memory usage before optimization: "
        ...
        for name, obj in self._registered_objects.items():
            if hasattr(obj, "optimize_memory") and callable(getattr(obj, "optimize_memory")):
                try:
        ...
        logger.info(f"Memory usage after optimization: "
    ...
    def start_monitoring(self, interval_seconds: float = 60.0) -> None:
            ...
            while not self._stop_event.is_set():
                try:
                    ...
                    if system_percent > self.max_memory_percent or process_percent > (self.max_memory_percent * 0.8):
                        ...
                        logger.debug(f"Memory usage - System: {system_percent:.1f}%, "
                                  f"Process: {process_percent:.1f}% "
        ...
        self._monitoring_thread = threading.Thread(target=monitor_memory, daemon=True, name="MemoryMonitor")
