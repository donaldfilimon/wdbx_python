...
class DiagnosticsVisualizer:
    ...
    def _create_plotly_overview(
        ...
        fig = make_subplots(
            rows=2,
            cols=2,
            subplot_titles=("Memory Usage (%)", "CPU Usage (%)", "Disk Usage (%)", "Query Count"),
    ...
    def create_component_dashboard(
        ...
        if component_names:
            components = {
                name: data for name, data in components.items() if name in component_names
            }
        ...
        if use_plotly and PLOTLY_AVAILABLE:
            return self._create_plotly_component_dashboard(
                components, datetime_labels, output_file, fig_size, dark_mode
            )
        if MATPLOTLIB_AVAILABLE:
    ...
    def create_events_timeline(
        ...
        if use_plotly and PLOTLY_AVAILABLE:
            return self._create_plotly_events_timeline(
                events, timestamps, types, type_colors, output_file, fig_size, dark_mode
            )
        if MATPLOTLIB_AVAILABLE:
        ...
        for event_type in set(types):
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    name=event_type,
                    marker=dict(
                        color=type_colors.get(event_type, "#3498DB"), size=12, symbol="circle"
                    ),
