...
class MetricsExporter(ABC):
    ...
    def start(self) -> bool:
        ...
        self.exporter_thread = Thread(
            target=self._export_loop,
            daemon=True
        )
        self.exporter_thread.start()
        logger.info(
            f"Started {self.__class__.__name__} with interval: {self.export_interval}s"
        )
        return True
...
class PrometheusExporter(MetricsExporter):
    ...
    def _convert_to_prometheus(self, metrics: Dict[str, Any]) -> List[str]:
        ...
        lines.append(f"wdbx_peak_memory_percent{{{self._get_labels()}}} {metrics['peak_memory_percent']}")
        ...
        lines.append(f"wdbx_avg_query_latency_ms{{{self._get_labels()}}} {metrics['avg_query_latency_ms']}")
        ...
        if hasattr(self.monitor, "metrics") and "components" in self.monitor.metrics:
            for component_name, component_metrics in self.monitor.metrics["components"].items():
                ...
                if component_fields:
                    ...
                    lines.append(f"{component_measurement},{component_tags} {component_fields_str} {timestamp}")
