...
def setup_parser() -> argparse.ArgumentParser:
    ...
    init_parser.add_argument(
        "--force", action="store_true", help="Force initialization even if data directory exists"
    )
    ...
    create_vector_parser.add_argument(
        "--data", type=str, required=True, help="Vector data as JSON array"
    )
    create_vector_parser.add_argument("--metadata", type=str, help="Vector metadata as JSON object")
    create_vector_parser.add_argument(
        "--id", type=str, help="Vector ID (generated if not provided)"
    )
    ...
    create_block_parser.add_argument(
        "--data", type=str, required=True, help="Block data as JSON object"
    )
    create_block_parser.add_argument(
        "--vectors", type=str, help="List of vector IDs to include in the block"
    )
    ...
    search_parser.add_argument(
        "--output-format", type=str, default="text", choices=["text", "json"], help="Output format"
    )
    ...
    search_blocks_group.add_argument(
        "--query-data", type=str, help="Query vector data as JSON array"
    )
    search_blocks_group.add_argument("--query-text", type=str, help="Query text")
    search_blocks_parser.add_argument(
    ...
    get_parser.add_argument(
        "--type", type=str, required=True, choices=["vector", "block"], help="Type of object to get"
    )
    get_parser.add_argument(
        "--output-format", type=str, default="text", choices=["text", "json"], help="Output format"
    )
    ...
    export_parser.add_argument(
        "--format", type=str, default="json", choices=["json", "binary"], help="Export format"
    )
    ...
    import_parser.add_argument(
        "--format", type=str, default="json", choices=["json", "binary"], help="Import format"
    )
...
def handle_init(args: argparse.Namespace) -> int:
    ...
    if data_dir.exists() and not args.force:
        logger.error(
            f"Data directory '{data_dir}' already exists, use --force to initialize anyway"
        )
        return 1
                ...
                output.append(
                    {
                        ...
                        "data": block.data,
                        "vectors": (
                            [v.vector_id for v in block.embeddings] if block.embeddings else []
                        ),
                    }
                )
            print(json.dumps(output, indent=2))
    ...
    try:
        ...
        if "memory_usage" in stats_after:
            ...
            if "memory_usage" in stats_before:
                ...
                logger.info(
                    f"Memory reduction: {reduction / 1024 / 1024:.2f} MB ({percentage:.2f}%)"
                )
