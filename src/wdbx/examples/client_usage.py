...
from wdbx.utils.logging_utils import get_logger, setup_logging
...
def example_context_manager():
    ...
    with wdbx_session(vector_dimension=384) as client:
        ...
        metadata_list = [{"index": i, "description": f"Batch vector {i}"} for i in range(5)]

        # Create vectors in batch
        vectors = client.batch_create_vectors(
            vector_data_list=vector_data_list, metadata_list=metadata_list
        )
...
def example_memory_optimization():
    ...
    with wdbx_session(vector_dimension=384, enable_memory_optimization=True) as client:
        ...
        for i in range(100):
            vector = client.create_vector(
                vector_data=np.random.rand(384).astype(np.float32), metadata={"index": i}
            )
        ...
        logger.info(
            f"Memory usage before optimization: {stats_before.get('memory_usage', 'N/A')} bytes"
        )
        ...
        logger.info(
            f"Memory usage after optimization: {stats_after.get('memory_usage', 'N/A')} bytes"
        )
    ...
    async def async_operations():
        with wdbx_session(vector_dimension=384) as client:
            ...
            vector = await async_client.create_vector(
                vector_data=vector_data, metadata={"description": "Async vector"}
            )
            ...
            similar_vectors = await async_client.find_similar_vectors(query_vector=vector, top_k=5)
            ...
            similar_blocks = await async_client.search_blocks(query=vector, top_k=3)
