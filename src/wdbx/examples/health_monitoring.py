...
from wdbx.health import print_health_report
    ...
    for i in range(iterations):
        try:
            ...
            with metrics.time_search_operation():
                results = wdbx_instance.find_similar_vectors(query_vector=query_vector, top_k=5)
...
def run_example(
    ...
    wdbx = WDBX(vector_dim=vector_dim, data_dir=data_dir, debug_mode=True)
    ...
    health_thread = Thread(
        target=monitor_health, args=(wdbx, health_interval, duration), daemon=True
    )
    health_thread.start()

    # Simulate load
    simulate_load(wdbx_instance=wdbx, iterations=load_iterations, vector_dim=vector_dim)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--data-dir", default="./data/health_example", help="Directory for data storage"
    )
    parser.add_argument("--vector-dim", type=int, default=128, help="Dimension of vectors")
    parser.add_argument(
        "--load-iterations", type=int, default=100, help="Number of iterations for load simulation"
    )
    parser.add_argument(
        "--prometheus-port", type=int, default=9090, help="Port for Prometheus metrics server"
    )
    parser.add_argument(
        "--health-interval", type=int, default=10, help="Interval between health checks in seconds"
    )
    parser.add_argument(
        "--duration",
        type=int,
        default=None,
        help="Duration to run in seconds (default: until load simulation completes)",
    )
