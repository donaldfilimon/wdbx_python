...
from wdbx.core.data_structures import EmbeddingVector, Block
...
def create_test_blocks(wdbx: WDBXCore, vectors: list[EmbeddingVector], count: int = 3):
    ...
    for i in range(count):
        ...
        # Create block
        block = wdbx.create_block(
            data=data,
            embeddings=block_vectors,
            block_id=f"block-{i}"
        )
...
def demonstrate_vector_operations(vectors: list[EmbeddingVector]):
    ...
    logger.info(f"Cosine similarity between '{vector1.metadata['name']}' and '{vector2.metadata['name']}': {similarity:.4f}")
    
    # Calculate Euclidean distance
    distance = vector1.euclidean_distance(vector2)
    logger.info(f"Euclidean distance between '{vector1.metadata['name']}' and '{vector2.metadata['name']}': {distance:.4f}")
...
def demonstrate_search(wdbx: WDBXCore, query_vector: EmbeddingVector):
    ...
    logger.info(f"Found {len(similar_vectors)} vectors similar to '{query_vector.metadata['name']}':")
    ...
    logger.info(f"Found {len(similar_blocks)} blocks relevant to '{query_vector.metadata['name']}':")
...
def demonstrate_plugin_integration():
    ...
    try:
        ...
        if discord_plugin:
            ...
            logger.info("Discord bot plugin found. To use it, create a config file and initialize the plugin.")
