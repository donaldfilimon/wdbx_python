...
def benchmark_operation(
    operation_name: str, operation_func: callable, iterations: int = 5, warmup: int = 2
) -> Tuple[float, float]:
    """
...
def benchmark_vector_operations(
    ml_backend: Any, dimensions: List[int] = [128, 768, 1536], batch_sizes: List[int] = [100, 1000]
...
def main() -> int:
    ...
    if args.benchmark:
        ...
        _, _ = benchmark_operation(
            "batch_cosine_similarity", lambda: ml_backend.batch_cosine_similarity(vector1, vectors)
        )
