...
def sample_embeddings():
    ...
    for i in range(50):
        ...
        embeddings.append(EmbeddingVector(vector=vector, metadata={"cluster": cluster, "index": i}))
    return embeddings
...
async def test_planner_distributes_queries(wdbx_instance, sample_embeddings):
    ...
    results = await wdbx_instance.search_similar_vectors(
        query_vector, top_k=min(10, len(vector_ids))
    )
    ...
    start_time = time.time()
    cached_results = await wdbx_instance.search_similar_vectors(
        query_vector, top_k=min(10, len(vector_ids))
    )
...
async def test_parallel_queries(wdbx_instance, sample_embeddings):
    ...
    start_time = time.time()
    tasks = [wdbx_instance.search_similar_vectors(vector, top_k=5) for vector in query_vectors]
