...
class MVCCTransaction:
    ...
    def acquire_lock(self, key: str) -> bool:
        ...
        if self.status != "active":
            logger.warning(f"Transaction {self.transaction_id} attempted to acquire lock while {self.status}")
            return False
        self.locks.add(key)
        return True
    ...
    def commit(self) -> None:
        ...
        self.release_locks()
        logger.debug(f"Transaction {self.transaction_id} committed after {self.end_time - self.start_time:.3f}s")
    ...
    def abort(self) -> None:
        ...
        self.end_time = time.time()
        self.release_locks()
        logger.debug(f"Transaction {self.transaction_id} aborted after {self.end_time - self.start_time:.3f}s")
...
class MVCCManager:
    ...
    def read(self, transaction_id: str, key: str) -> Optional[Any]:
        ...
        with self.lock:
            ...
            if not transaction.is_active():
                raise ValueError(f"Transaction {transaction_id} is not active (status: {transaction.status})")
            ...
            valid_versions = [
                (v, val, tid) for v, val, tid in versions 
                if (v <= transaction.version and 
                    (tid == transaction_id or self.transactions.get(tid, MVCCTransaction()).is_committed()))
                or tid == transaction_id
            ]

            if not valid_versions:
                logger.debug(f"Transaction {transaction_id} read None for key {key} (no valid versions)")
                return None

            # Return the most recent valid version
            valid_versions.sort(reverse=True, key=lambda x: x[0])
            logger.debug(f"Transaction {transaction_id} read value for key {key} (version: {valid_versions[0][0]})")
    ...
    def write(self, transaction_id: str, key: str, value: Any) -> bool:
        ...
        with self.lock:
            ...
            if key in self.locks and self.locks[key] != transaction_id:
                logger.debug(f"Transaction {transaction_id} failed to write to {key}: locked by {self.locks[key]}")
                return False
            ...
            logger.debug(f"Transaction {transaction_id} wrote value for key {key} (version: {transaction.version})")
            return True
    ...
    def commit(self, transaction_id: str) -> bool:
        ...
        with self.lock:
            ...
            if not transaction.is_active():
                logger.warning(f"Attempt to commit {transaction.status} transaction: {transaction_id}")
                return False
            ...
            logger.info(f"Transaction {transaction_id} committed: {len(transaction.write_set)} writes, {len(transaction.read_set)} reads")
    ...
    def abort(self, transaction_id: str) -> bool:
        ...
        with self.lock:
            ...
            if not transaction.is_active():
                logger.warning(f"Attempt to abort {transaction.status} transaction: {transaction_id}")
                return False
            ...
            logger.info(f"Transaction {transaction_id} aborted: {len(transaction.write_set)} writes rolled back")
            return True

    def _auto_cleanup(self, cleanup_interval: float = 3600.0) -> None:
        """
        Perform automatic cleanup if enough time has passed since last cleanup.
        ...
        if current_time - self.last_cleanup_time > cleanup_interval:
            removed = self.cleanup_old_versions()
            if removed > 0:
                logger.info(f"Automatic cleanup removed {removed} old versions")
    ...
    def cleanup_old_versions(self, max_age: float = 3600.0) -> int:
        ...
        with self.lock:
            ...
            old_transactions = [tid for tid, tx in self.transactions.items() 
            ...
            for key in list(self.versions.keys()):
                ...
                committed_versions = [v for v in versions 
            ...
            logger.debug(f"Cleaned up {removed_count} old versions, current keys: {len(self.versions)}")
