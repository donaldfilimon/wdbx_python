                ...
                for _, obj in inspect.getmembers(module):
                    if (inspect.isclass(obj) and
                       ...
                       hasattr(obj, "initialize")):
                        ...
                        logger.debug(
                            f"Discovered plugin: {obj.NAME} "
                            f"(v{obj.VERSION}): {obj.DESCRIPTION}"
                        )

            except ImportError as e:
                logger.error(f"Import error loading plugin from {file_path}: {e}")
...
def initialize_all_plugins(config: Optional[Dict[str, Any]] = None) -> Dict[str, WDBXPlugin]:
    """
...
def get_all_launch_configurations() -> Dict[str, Any]:
                ...
                if examples and isinstance(examples, list):
                    for config in examples:
                        if config not in launch_config["configurations"]:
                            # Cast to ensure type checking passes
                            configs = cast(List, launch_config["configurations"])
    ...
    if launch_config["configurations"]:
        compounds = cast(List, launch_config["compounds"])
        compounds.append({
            "name": "WDBX: Start All Plugins",
            "configurations": [
                cast(Dict[str, Any], config)["name"]
                for config in launch_config["configurations"]
                if "Debug" not in cast(Dict[str, Any], config)["name"]
            ]
        })
...
if __name__ == "__main__":
    ...
    launch_config_parser = subparsers.add_parser(
        "launch-config",
        help="Generate VS Code launch configurations"
    )
    launch_config_parser.add_argument("--output", "-o", type=str, help="Output file path")
    launch_config_parser.add_argument(
